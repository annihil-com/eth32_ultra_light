...
void CGui::Init(void)
{
	...
	winPicker->AddWindow("Visuals", CAT_OTHER);
	winPicker->AddWindow("Chams", CAT_OTHER);
	winPicker->AddWindow("Colors", CAT_OTHER);
	winPicker->AddWindow("GUI", CAT_OTHER);
	winPicker->AddWindow("Misc", CAT_OTHER);
	winPicker->ClearAll();
...
void CGui::InitStyles(void)
{
	...
	sprintf_s(styleFile, MAX_PATH, "%s\\GameData\\Settings\\%s", eth32.path, GUI_STYLES_FILE);
