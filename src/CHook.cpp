	...
	if (strstr(lpLibName, "cgame_mp_x86.dll"))
	{
		...
		if (eth32.cg.supported == -1)
		{
			...
			if (strstr(lpLibName, "etmain") || strstr(lpLibName, "etpub") || strstr(lpLibName, "shrub"))
			{
				...
				sprintf_s(automoddll, sizeof(automoddll),"%s%s", eth32.path, "GameData\\Offsets\\cgame_mp_x86.dll");
			...
			TRASH108
			TRASH105
			orig_syscall(UI_ERROR, "This mod isn't supported.");
			TRASH99
		}
		else
		{
...
int __cdecl eth32_CG_syscall(int command, ...)
{
	...
	switch (command)
	{
		...
		case CG_GETSERVERCOMMAND:
		{
			...
			if (eth32.cgMod->type != MOD_ETPRO && eth32.settings.autoPredBots) {	
				if ( !strcmp( cmd, "sc0" ) ) {
