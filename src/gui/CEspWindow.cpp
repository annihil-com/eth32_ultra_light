...
void CEspWindow::Display(void)
{
	...
	else if (displayClientNum > -1 && (time - timeStart) < eth32.settings.xhairInfoTime) {
		show = true;
	}
	else {
		show = false;
	...
	if (show) {
		...
		strncpy_s(name, 128, player->name, cnt);
		...
		sprintf_s(p,sizeof(p),"%.2f", player->stats.kdRatio);
		...
		if (player->stats.bulletsFired)
			sprintf_s(p,sizeof(p),"%.1f", ((float)player->stats.bulletsHit/player->stats.bulletsFired)*100.f);
		else sprintf_s(p,sizeof(p),"0.0");
		Draw.Text(drawX, drawY, GUI_FONTSCALE, "Accuracy", GUI_FONTCOLOR1, qtrue, qfalse, GUI_FONT, true);
		Draw.Text(drawX2 - Draw.sizeTextWidth(GUI_FONTSCALE, p, GUI_FONT), drawY, GUI_FONTSCALE, p, GUI_FONTCOLOR2, qtrue, qfalse, GUI_FONT, true);

		drawY += GUI_FONTHEIGHT + GUI_TEXTSPACING;

		sprintf_s(p,sizeof(p),"%.1f", player->stats.threat);
