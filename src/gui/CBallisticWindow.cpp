...
void CBallisticWindow::Display(void)
{
	...
	sprintf_s(buffer, sizeof(buffer), "%.2f", 0.001*(float)eth32.cg.snap->ps.grenadeTimeLeft);
	...
	if (Aimbot.grenadeTarget) 
		strncpy_s(buffer, sizeof(buffer), Aimbot.grenadeTarget->name, Draw.BreakStringByWidth(Aimbot.grenadeTarget->name, w - 15 - TEXTWIDTH("Target:"), NULL, GUI_FONT, false));
	else
		strcpy_s(buffer, sizeof(buffer), "<none>");
	...
	sprintf_s(buffer, sizeof(buffer), "%i", Aimbot.grenadeTarget ? (int)Aimbot.grenadeTarget->distance : -1);
	Draw.Text(x + 5, yPos, GUI_FONTSCALE, "Distance:", GUI_FONTCOLOR1, qtrue, qfalse, GUI_FONT, true);
	Draw.Text(x2 - 5 - TEXTWIDTH(buffer), yPos, GUI_FONTSCALE, buffer, GUI_FONTCOLOR2, qtrue, qfalse, GUI_FONT, true);
	yPos += GUI_FONTHEIGHT + GUI_TEXTSPACING;

	sprintf_s(buffer, sizeof(buffer), "%.1f s", Aimbot.grenadeTarget ? Aimbot.rifleGrenadeTime : 0.0); 
